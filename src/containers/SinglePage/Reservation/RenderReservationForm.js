...
const RenderReservationForm = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert(
      `Ngày nhận phòng: ${formState.startDate}\nNgày trả phòng: ${formState.endDate}\nRooms: ${formState.room}\nGuests: ${formState.guest}`
    );
  };
    ...
    <ReservationFormWrapper className="form-container" onSubmit={handleSubmit}>
      ...
      <FieldWrapper>
        ...
        <ViewWithPopup
          ...
          view={
            <Button type="default">
              <span>Số phòng {formState.room > 0 && `: ${formState.room}`}</span>
              <span>-</span>
              <span>Số khách{formState.guest > 0 && `: ${formState.guest}`}</span>
            </Button>
          }
          popup={
            <RoomGuestWrapper>
